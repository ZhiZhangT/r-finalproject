...
As noticed in the time-series analysis, the prices of HDB flats changed greatly over time. Let's compare the price of various flat types over a shorter time frame to see if the above observation holds. We chose data from 2022 for this as this is the latest full-year-data we have.
...
```
Comparing the above three plots for 2022 against their corresponding plots for the entire time period, we see that the overall trends are very similar. 
...
### Resale Price vs Area

Let's analyse how the resale price relates to floor area. As the resale price is heavily affected by the year of sale, we shall only use data from 2022 as this is the latest year with full data. 
```{r}
ggplot(data = clean_resale_data_2022, aes(x=floor_area_sqm, y=resale_price)) +
  geom_point() +
  geom_smooth() +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Resale Price Over Area", x = "Floor Area (sqm)", y = "Resale Price")

```
Looking at the graph, we can see a rather linear relationship between Resale Price and Floor Area.

As the resale price is also affected by the town, let's explore how the various towns differ
```{r}
ggplot(data = clean_resale_data_2022, aes(x=floor_area_sqm, y=resale_price, color=town)) +
...
ggplot(clean_resale_data, aes(x = remaining_lease, y = resale_price)) +
  ...
  labs(title = "Resale Prices vs. Remaining Lease", x = "Remaining Lease (Years)", y = "Resale Price (SGD)")
```
From this scatter plot, we do not observe a clear trend. This is because aside from remaining lease, there are many other factors that affect the resale price.

Thus, let us only consider more recent resale data, from 2020 onwards.
...
Let's also look at how the variance of the resale price changes with Remaining Lease

```{r}
var_price_per_year <- clean_resale_data %>%
  group_by(remaining_lease) %>%
  summarise(price_var = var(resale_price))

ggplot(var_price_per_year, aes(x = remaining_lease, y = price_var)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Variance of Resale Price vs. Remaining Lease", x = "Remaining Lease (Years)", y = "Resale Price Variance")
```
From this, we can see that the variance of the resale price increases as the lease drops from 99 years to around 70 years. Thereafter, the variance decreases. 


### Predict the future price of HDB for Remaining lease years of less than 42 years 
From the sales data we are utilising, we can see that the lowest remaining lease value is 42 years. This is because it has only been around 60 years since the first HDB unit was sold. As such, til date, there has been no unit that was returned to the government. We know that the price of these units will be zero when the lease runs out. As such, one point of interest we can analyse would be whether this fact is taken into account by the resale market.
...
clean_resale_data_2020_onwards_00 <- bind_rows(clean_resale_data_2020_onwards, additional_point)

lm_model <- lm(resale_price ~ remaining_lease, data = clean_resale_data_2020_onwards_00)

weights <- ifelse(clean_resale_data_2020_onwards_00$remaining_lease == 0, 20509, 1)

weighted_lm_model <- lm(resale_price ~ remaining_lease, data = clean_resale_data_2020_onwards_00, weights = weights)
...
```{r}
# Scatterplot of data
plot(clean_resale_data_2020_onwards_00$remaining_lease, clean_resale_data_2020_onwards_00$resale_price, pch = 19, col = "black", main = "Weighted Regression Line: Resale Prices vs. Remaining Lease", xlab = "Remaining Lease (Years)", ylab = "Resale Price (SGD)")
...
abline(weighted_lm_model, col = "red")
```
In the above linear regression, that the trend based on current prices (blue line) has an R square value of 0.1158. This means that a linear model might not be a good estimate of the trend. 
The red line take the fact that the price will be 0 when the remaining lease hits 0 into consideration. This has an R square value of 0.6347. 

Since the linear regression model does not provide a good prediction of trend, we shall try a quadratic regression model.

Price prediction using ggtrendline's quadratic regression function
```{r}
library(ggtrendline)
ggtrendline(clean_resale_data_2020_onwards_00$remaining_lease, clean_resale_data_2020_onwards_00$resale_price, model = "line3P")
```
Plotting onto the same graph,
```{r}
eq = function(x){-160*x*x + 27747*x - 621382}
ggplot(clean_resale_data_2020_onwards_00, aes(x = remaining_lease, y = resale_price)) +
  geom_point() +
  stat_function(fun=eq)
  labs(title = "Scatterplot of Resale Prices vs. Remaining Lease", x = "Remaining Lease (Years)", y = "Resale Price (SGD)")
  
```
This quadratic regression model has a poor R squared value of 0.146, meaning that the relationship might not be quadratic as well. 

Overall, as the resale price is affected by many other factors such as location and floor area, we might have to consider utilising multi-variable regression instead.
