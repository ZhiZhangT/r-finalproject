...
Let us now train a logistic regression model using the same train_data
```{r}
# Define a function for polynomial regression
poly_reg <- function(data, response) {
  predictors <- setdiff(names(data), response)
  formula <- as.formula(paste(response, "~ poly(", paste(predictors, collapse = ", "), ", degree = 2)"))
  model <- lm(formula, data = data)
  return(summary(model))
}

# Use the function
model_summary <- poly_reg(train_data, "resale_price")
