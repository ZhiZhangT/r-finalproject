...
For **"storey_range"**: Since the storey data is provided in a range, we shall determine the average storey and use that for analysis.
...
Looking at the above histogram, we notice an anomaly in the price distribution of 2 room flats, with two peaks at around SGD 120,000 and SGD 250,000. Upon further research, we found out that two room flats come in two types: Type 1 and Type 2. The more affordable type 1 flats are generally catered towards lower income households. This separation between type 1 and 2 two room flats is not delineated in our dataset. <https://www.hdb.gov.sg/cs/infoweb/-/media/doc/CCG/04022021-BTO-Annexes/Annex-A2-Feb-2021-BTO.pdf>
...
These areas are primarily residential estates with a large amounts of HDB flats, which will explain why these towns made up most of the resale transactions during peaks.
...
Here, we aim to find out the town that has the greatest gains in price over time:

```{r}
monthly_prices <- clean_resale_data %>%
  group_by(month, town) %>%
  summarize(mean_resale_price = mean(resale_price)) %>%
  arrange(mean_resale_price, DESC=T)

ggplot(monthly_prices) +
  scale_y_continuous("Resale Price", labels = scales::comma ) +
  geom_smooth(aes(x=month, y=mean_resale_price, color=town))
```

We will also calculate the average YoY gains for each town over time:

```{r}
# Calculate the average year over year gain for each town
avg_gains <- monthly_prices %>%
  group_by(town) %>%
  mutate(price_last_year = lag(mean_resale_price, 12), 
         YoY_gain = (mean_resale_price - price_last_year) / 
           price_last_year * 100) %>%
  summarise(avg_YoY_gain = mean(YoY_gain, na.rm = TRUE))

avg_gains %>%
  arrange(avg_YoY_gain, DESC=T)

view(avg_gains)
```

From the results, we see that Lim Chu Kang has the highest average YoY gains at 54%, which is considered as an outlier because all flats in the town were bought over by the government in 2002. Next up we have Sembawang and Sengkang which started out as relatively immature estates, but then developed over time to become mature estates which explained the high gains in prices.
...
We duplicate the clean_resale_data so that we can clean up the data for linear regression while keeping the original file 
```{r}
duplicate_resale_data <- clean_resale_data
```

To simplify the linear regression model, we remove some of the variables from the clean_resale_data dataset.

```{r}
lm_resale_data <- duplicate_resale_data[, !colnames(duplicate_resale_data) %in% c("block", "street_name", "storey_min", "storey_max", "storey_range", "lease_commence_date")]
colnames(lm_resale_data)
```

We check the class of data within each column for duplicate_resale_data
```{r}
str(lm_resale_data)
```
We notice that 'town' and 'flat_type' are factors. We want to convert them into dummy variables for linear regression.
```{r}
# For 'town' variable
town_dummy_var <- model.matrix(~town - 1, data = lm_resale_data)
town_dummy_var_df <- as.data.frame(town_dummy_var)
lm_resale_data <- cbind(lm_resale_data, town_dummy_var_df)

# For 'flat_type' variable
flat_type_dummy_var <- model.matrix(~flat_type - 1, data = lm_resale_data)
flat_type_dummy_var_df <- as.data.frame(flat_type_dummy_var)
lm_resale_data <- cbind(lm_resale_data, flat_type_dummy_var_df)

lm_resale_data <- subset(lm_resale_data, select = -town)
lm_resale_data <- subset(lm_resale_data, select = -flat_type)
```

We also need to conver the 'date' variable from character to numerical.
```{r}
# Assuming 'month_var' is your month variable
lm_resale_data$month <- as.Date(paste(lm_resale_data$month, "-01", sep=""), format = "%Y-%m-%d")
lm_resale_data$month <- as.numeric(lm_resale_data$month)
```


Now let us check the variables in the dataset 'lm_resale-data'
```{r}
str(lm_resale_data)
```

Here we split the entire data set lm_resale_data into both training data (90% data) and test data (10% data).

```{r}
set.seed(123)
train_proportion <- 0.9
total_rows <- nrow(lm_resale_data)
train_size <- round(train_proportion * total_rows)
test_size <- total_rows - train_size
sample_index <- sample(1:total_rows, total_rows)
train_data <- lm_resale_data[sample_index[1:train_size], ]
test_data <- lm_resale_data[sample_index[(train_size + 1):total_rows], ]
summary(train_data)
```
...
```{r}
lm_model_twov <- lm(resale_price ~ floor_area_sqm + remaining_lease, data = train_data)
...
```
Now let us create a machine learning model with all the variables. We first create a 
```{r}
lm_model_allv <- lm(resale_price ~ . - resale_price, data = train_data)
```
...
Now we will use the train_data to train a logistic regression model. We first normalize the output variable 'resale_price'
```{r}
train_data$resale_price <- (train_data$resale_price - min(train_data$resale_price)) / 
                          (max(train_data$resale_price) - min(train_data$resale_price))
test_data$resale_price <- (test_data$resale_price - min(test_data$resale_price)) / 
                          (max(test_data$resale_price) - min(test_data$resale_price))
logistic_model <- glm(resale_price ~ ., data = train_data, family = binomial)
summary(logistic_model)
```

Now we test the model with test_data
```{r}
# Assuming 'test_data' is your test data frame and 'resale_price' is your binary outcome variable

predictions <- predict(logistic_model, newdata = test_data, type = "response")
class_predictions <- ifelse(predictions > 0.5, 1, 0)
errors <- class_predictions - test_data$resale_price
mae <- mean(abs(errors))
mse <- mean(errors^2)
rmse <- sqrt(mse)
...
```
We now scale up the mean absolute error data from the normalised value of 0.2

K Nearest Neighbour

```{r}
min_value <- min(clean_resale_data$resale_price)
max_value <- max(clean_resale_data$resale_price)
mean_absolute_error_logistical <- (0.205*(max_value-min_value))
mean_absolute_error_logistical
```

Comparing between the mean absolute error of the linear regression model and the logistic regression model, we find that the linear regression and logistical regression model have errors of 54000 and 306000 respectively. The linear regression model performs much better than the logistic regression model likely due to a few reasons:

1. Linear regression is designed to predict a continuous outcome variable, while logistic regression is     designed to predict a binary outcome variable. If resale_price is a continuous variable, then it’s more appropriate to use linear regression. Using logistic regression to predict a continuous variable requires transforming the variable into a binary one, which can lead to loss of information.

2. Linear regression assumes a linear relationship between the independent and dependent variables, while logistic regression does not. If the true relationship is linear, then linear regression will likely perform better.

3. Logistic regression, especially with many predictor variables, can sometimes overfit the training data, leading to poor performance on the test data.

4. Mean absolute error (MAE) is a common error metric for regression problems, but it might not be the most appropriate metric for a classification problem like the one solved by logistic regression. For classification problems, other metrics like accuracy, precision, recall, or area under the ROC curve might be more informative.
...
Cleaning data,
```{r}
profit_over_years_clean <- profit_over_years %>% 
  select(-financial_year) %>% 
  select(-min_selling_price) %>%
  select(-max_selling_price) %>% 
  select(-min_selling_price_less_ahg_shg) %>%
  select(-max_selling_price_less_ahg_shg) %>%
  select(-n) %>%
  select(-flat_type) %>%
  select(-room_type)
...
colnames(profit_over_years_clean)

profit_over_years_long <- profit_over_years_clean %>%
  gather(key = "Description", value = "Profit", `Mean Resale Profit  1  years after the MOP`, `Mean Resale Profit  2  years after the MOP`, `Mean Resale Profit  3  years after the MOP`, `Mean Resale Profit  4  years after the MOP`, `Mean Resale Profit  5  years after the MOP`, `Mean Resale Profit  6  years after the MOP`, `Mean Resale Profit  7  years after the MOP`, `Mean Resale Profit  8  years after the MOP`, `Mean Resale Profit  9  years after the MOP`, `Mean Resale Profit  10  years after the MOP`)

profit_over_years_long
```

I wanted to plot the profit in each year after MOP for each town, but did not manage to do so before the deadline. Hence this will be included in our Future Work.
